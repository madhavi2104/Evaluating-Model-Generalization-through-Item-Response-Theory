...
HEAVY_OVERRIDES = {
    "beit_base_patch16_224": {"batch_size": 16, "workers": 4},
    "cait_m36_384": {"batch_size": 8, "workers": 4},
    "eva_giant_patch14_224": {"batch_size": 4, "workers": 4},
    "maxvit_tiny_tf_512": {"batch_size": 4, "workers": 4},
    "convnext_base": {"batch_size": 32, "workers": 8},
    "convnextv2_large": {"batch_size": 24, "workers": 8},
    "convmixer_768_32": {"batch_size": 32, "workers": 8},
    "swinv2_tiny_window16_256": {"batch_size": 32, "workers": 8},
    "swinv2_base_window8_256": {"batch_size": 24, "workers": 8},
    "coatnet_0_rw_224": {"batch_size": 32, "workers": 8},
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Launch TIMM training with repo-safe paths."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional explicit path to the TIMM training script.",
    )
...
def resolve_train_script(arg_value: Path | None, repo_root: Path) -> Path:
    ...
    return (repo_root / "src" / "training" / "backends" / "timm" / "train.py").resolve()
...
def resolve_output_dir(output_root: Path, dataset: str, model: str) -> Path:
    return (output_root / "training_timm" / dataset / model).resolve()


def resolve_hyperparameters(model: str, dataset: str) -> tuple[int, int, int]:
    batch_size = 64
    workers = 8
    num_classes = 1000
    ...
    if any(key in model for key in ["cait", "eva"]):
        batch_size = min(batch_size, 8)
    elif "maxvit" in model:
        batch_size = min(batch_size, 4)
    elif "beit" in model:
        batch_size = min(batch_size, 16)

    if model in HEAVY_OVERRIDES:
        batch_size = HEAVY_OVERRIDES[model]["batch_size"]
        workers = HEAVY_OVERRIDES[model]["workers"]

    return batch_size, workers, num_classes
...
def main() -> None:
    ...
    batch_size, workers, num_classes = resolve_hyperparameters(
        args.model, args.dataset
    )

    print(
        f"Starting TIMM wrapper for model={args.model} | dataset={args.dataset}",
        flush=True,
    )
    ...
    print(
        f"Resolved settings -> batch_size={batch_size}, "
        f"workers={workers}, num_classes={num_classes}",
        flush=True,
    )
