...
output_root <- paste0("E:/Thesis/IRTNet/output/Hypothesis_2/analysis/IRT_based/anchor_stability_", pool_mode)
dir.create(output_root, recursive = TRUE, showWarnings = FALSE)

load_flipped_data <- function(test_file) {
  df <- read.csv(test_file)
  
  binary_data <- df[, -(1:2)]
  binary_data <- as.data.frame(lapply(binary_data, function(x) as.numeric(as.character(x))))
  
  keep_rows <- complete.cases(binary_data)
  binary_data <- binary_data[keep_rows, ]
  item_names <- df$Item[keep_rows]
...
make_item_stats <- function(X) {
  p <- colMeans(X)
  v <- apply(X, 2, var)
  data.frame(Item = names(p), p = as.numeric(p), var = as.numeric(v), stringsAsFactors = FALSE)
}

filter_pool <- function(X, dataset, mode) {
  stats <- make_item_stats(X)
  
  if (pool_mode == "full") {
    keep <- stats$Item
  } else {
    pars <- get_pool_params(dataset, mode)
    keep <- stats %>%
...
# FIXED: uses df$p consistently and computes cut points once
select_anchors_stratified <- function(stats_df, anchor_n) {
  ...
  pick_topvar <- function(pool, n) {
    pool %>% arrange(desc(var), Item) %>% slice_head(n = n) %>% pull(Item)
  }
  ...
  if (length(anchors) < anchor_n) {
    remaining <- df %>% filter(!Item %in% anchors) %>% arrange(desc(var), Item)
...
extract_ab <- function(fit, anchor_items) {
  ...
  a_col <- if ("a" %in% cols) "a" else if ("a1" %in% cols) "a1" else cols[grepl("^a", cols)][1]
  b_col <- if ("b" %in% cols) "b" else if ("d" %in% cols) "d" else cols[grepl("^b|^d", cols)][1]
...
# More robust N* rule: first N where med_delta < eps and stays below for all later N
...
for (test_file in test_files) {
  cat("\n=============================\n")
  cat("Processing:", test_file, "\n")
  
  base <- file_path_sans_ext(basename(test_file))
  parts <- strsplit(base, "_")[[1]]
  dataset <- parts[1]
  mode <- ifelse(str_detect(base, "trained"), "trained", "zeroshot")
  
  out_dir <- file.path(output_root, dataset, mode)
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
  
  X_raw <- load_flipped_data(test_file)
  cat("Raw dims (models x items):", dim(X_raw), "\n")
  
  pool <- filter_pool(X_raw, dataset, mode)
  X0 <- pool$X
  stats0 <- pool$stats
  cat("Pool dims after mode=", pool_mode, ":", dim(X0), "\n")
  
  if (ncol(X0) < anchor_n + 10) {
    warning("Too few usable items in pool for ", base, ". Skipping.")
    next
  }
  ...
  for (N in valid_sizes) {
    ...
    for (sd in seeds) {
      ...
      extras <- if (extras_needed > 0) sample(remaining, extras_needed) else character(0)
  ...
  if (nrow(df) == 0) {
    warning("No valid fits extracted for ", base)
    next
  }
  ...
  p_b <- ggplot(df, aes(x = SampleSize, y = b, group = interaction(Item, Seed))) +
    ...
    facet_wrap(~ Seed) +
    labs(title = paste0(base, " | Anchor difficulty stability (b)"),
         subtitle = paste0("pool_mode=", pool_mode, " | ", anchor_n, " anchors"),
         x = "Sample size (items)", y = "b (difficulty)") +
    theme_minimal()
  ggsave(file.path(out_dir, "anchor_b_trends_by_seed.pdf"), p_b, width = 10, height = 5)
  
  p_a <- ggplot(df, aes(x = SampleSize, y = a, group = interaction(Item, Seed))) +
    geom_line(alpha = 0.35) +
    facet_wrap(~ Seed) +
    labs(title = paste0(base, " | Anchor discrimination stability (a)"),
         subtitle = paste0("pool_mode=", pool_mode, " | ", anchor_n, " anchors"),
         x = "Sample size (items)", y = "a (discrimination)") +
    theme_minimal()
  ggsave(file.path(out_dir, "anchor_a_trends_by_seed.pdf"), p_a, width = 10, height = 5)
  ...
  fwrite(data.frame(
    Dataset = dataset, Mode = mode, File = test_file,
    pool_mode = pool_mode,
    eps_b = eps_b, N_star_b = thr_b$N_star,
    eps_a = eps_a, N_star_a = thr_a$N_star
