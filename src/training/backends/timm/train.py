...
group.add_argument('--early-stop-patience', type=int, default=0,
                   help='Stop training if the best eval metric does not improve for N epochs (default: 0 / disabled)')
...
# compute input channels (must be top-level, not nested under fast_norm)
    in_chans = 3
    ...
    # ---- ALWAYS build factory_kwargs, then ALWAYS create the model ----
    factory_kwargs = {}
    ...
    if args.model == "mnasnet1_0":
        # Your torchvision MNASNet special case
        from torchvision.models.mnasnet import MNASNet
        model = MNASNet(alpha=1.0, num_classes=args.num_classes, version=1)
    else:
        model = create_model(
            args.model,
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.early_stop_patience and best_epoch is not None:
                if (epoch - best_epoch) >= args.early_stop_patience:
                    _logger.info(
                        f"Early stopping: no improvement for {args.early_stop_patience} epochs "
                        f"(best epoch = {best_epoch}, best {eval_metric} = {best_metric:.4f})."
                    )
                    break
...
def train_one_epoch(
    ...
    if args.mixup_off_epoch and epoch >= args.mixup_off_epoch:
        if args.prefetcher and hasattr(loader, "mixup_enabled") and loader.mixup_enabled:
    ...
    base_accum = max(1, args.grad_accum_steps)
    updates_per_epoch = (len(loader) + base_accum - 1) // base_accum
    # size of the final (possibly shorter) accumulation window; always in [1, base_accum]
    last_accum = len(loader) - (updates_per_epoch - 1) * base_accum
    last_accum_start = len(loader) - last_accum
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        cur_accum = base_accum if batch_idx < last_accum_start else last_accum
        need_update = last_batch or ((batch_idx + 1) % cur_accum == 0)
        update_idx = num_updates % updates_per_epoch
        ...
        # multiply by cur_accum to get equivalent for full update
        data_time_m.update(cur_accum * (time.time() - data_start_time))
        ...
        losses_m.update(loss.item() * cur_accum, batch_size)
